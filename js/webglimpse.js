...
( function( root, factory ) {
	...
	Filter
	*/
	var META_IMAGES = [ 'msapplication-square152x152logo', 'msapplication-tileimage', 'twitter:image', '196x196', '180x180', '144x144', '96x96', 'thumbnail', 'primaryImageOfPage',
	...
	*/
	...
	/**
	  returns a websiteData object with intressting meta infos
	*/
    ...
    // check preloads
		...
		xhr.onreadystatechange = function( e ) {
			...
			websiteData.url = xhr.responseURL

			if ( xhr.status >= 400 || xhr.status == 0) return reject( websiteData )
			...
			if ( !rawMeta ) {
				...
				websiteData.title = stringToURL( websiteData.url ).hostname
			...
			// get image
			var image
			if ( image = META_IMAGES.find( tag => keys.indexOf( tag ) > -1 ) ) websiteData.iconLink = complete( meta[ image ], websiteData.url )
			else if ( image = FILTER_IMAGE_TAG( rawDoc ) ) websiteData.iconLink = complete( image, websiteData.url )
			else if ( image = META_IMAGES_SECOND.find( tag => keys.indexOf( tag ) > -1 ) ) websiteData.iconLink = complete( meta[ image ], websiteData.url )
	...
	*/
	...
	function tryUpgradeConnection( link ) {
		return new Promise( ( resolve, reject ) => {

      if(!link.startsWith('http')) var parts = stringToURL("https://" + link)
      else var parts = stringToURL(link)

			var withoutProtocol = `://${parts.host}${parts.pathname}${parts.search}`
			...
			xhr.onreadystatechange = function( e ) {
				...
				if ( xhr.status < 400 ) return resolve( "https" + withoutProtocol )
				else return resolve( "http" + withoutProtocol )

			}
			xhr.onerror = function() {
				return resolve( "http" + withoutProtocol)
			}

			xhr.open( "GET", "https" + withoutProtocol, true )
			xhr.send()
